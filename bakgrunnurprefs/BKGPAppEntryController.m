...
-(void)updateParentViewController{
    // 先尝试 PSListController 的私有 _parentController（标准 Preferences 框架流程）
    UIViewController *parentController = nil;
    @try {
        parentController = (UIViewController *)[self valueForKey:@"_parentController"];
    } @catch (NSException *e) {}
    ...
    // 手动 push 时：从 navigationController 的 viewControllers 栈中查找
    for (UIViewController *vc in self.navigationController.viewControllers) {
        if ([vc isKindOfClass:[BKGPApplicationListSubcontrollerController class]]) {
            [(BKGPApplicationListSubcontrollerController *)vc updateIvars];
            [(BKGPApplicationListSubcontrollerController *)vc reloadSpecifiers];
            return;
        }
    }
}
